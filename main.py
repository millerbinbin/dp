...
def crawl_shops():
    for i in open(CSV_DIR + "/base/category.csv"):
        category_name, category_code = i.strip().split(FIELD_DELIMITER)
        print "start to crawl: {0}...".format(category_name)
        info_list, heat_list, score_list, cmt_list = shop.crawl_all_shops_by_category(category_code, "taste", 8.5, 3)
        #csvLib.writeRecordsToFile(CSV_DIR + "/shops/{0}.csv".format(category), info_list, FIELD_DELIMITER)
        #csvLib.writeRecordsToFile(CSV_DIR + "/heats/{0}.csv".format(category), heat_list, FIELD_DELIMITER)
        #csvLib.writeRecordsToFile(CSV_DIR + "/scores/{0}.csv".format(category), score_list, FIELD_DELIMITER)
        #csvLib.writeRecordsToFile(CSV_DIR + "/comments/{0}.csv".format(category), cmt_list, FIELD_DELIMITER)
